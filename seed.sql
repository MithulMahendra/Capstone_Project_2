-- sql/seed.sql — Bootstrap NorthStar Bank Credit Card Spend Summarizer database

-- Run as a PostgreSQL superuser:
--   psql -U postgres -f seed.sql

-- ── Step 1: Create DB (run connected to default 'postgres' database) ─────────

-- Clean slate for development/testing (comment out in production)
DROP DATABASE IF EXISTS northstar_cc_db;
CREATE DATABASE northstar_cc_db;
...
GRANT CONNECT ON DATABASE northstar_cc_db TO rag_readonly;

-- ── Switch to target database ────────────────────────────────────────────────

\c northstar_cc_db

-- ── Step 3: Extensions & schema permissions ──────────────────────────────────
...
CREATE TABLE IF NOT EXISTS reward_transactions (
    reward_txn_id     UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    card_id           VARCHAR(20)   REFERENCES credit_cards(card_id),
    txn_date          DATE,
...
INSERT INTO credit_cards (
    card_id, customer_id, card_variant, credit_limit, available_limit, cash_limit,
    outstanding_amt, statement_date, due_date, min_due, reward_points, issued_date
) VALUES
...
-- 4. CARD TRANSACTIONS — CC-881001 (Primary account)

-- January 2026 (billing cycle Jan 26 – Feb 25)
INSERT INTO card_transactions (
    card_id, txn_date, posting_date, txn_type, amount, original_currency,
    original_amount, merchant_name, category_code, category_name,
    is_international, reward_pts_earned
) VALUES
...
-- 5. TRANSACTIONS — OTHER CARDS

INSERT INTO card_transactions (
    card_id, txn_date, posting_date, txn_type, amount, original_currency,
    merchant_name, category_code, category_name, is_international, reward_pts_earned
) VALUES
...
INSERT INTO reward_transactions (
    card_id, txn_date, points_earned, points_redeemed, points_expired, description, expiry_date
) VALUES
...
INSERT INTO billing_statements (
    card_id, billing_month, start_date, end_date, due_date,
    opening_balance, total_purchases, total_payments, total_fees,
    total_refunds, closing_balance, min_amount_due, reward_pts_earned
) VALUES
('CC-881001','2026-02','2026-01-26','2026-02-25','2026-03-15', 15000,  96800, 40000, 999.00,   0.00,  72799.00, 3640.00, 1332),
('CC-881001','2026-03','2026-02-26','2026-03-25','2026-04-15', 72799, 105400, 55000, 427.50, 2000.00, 121627.00, 6081.00, 1550),
('CC-882001','2026-03','2026-02-26','2026-03-25','2026-04-15', 20000,  76800, 80000,   0.00,   0.00,  16800.00,  840.00, 3708),
('CC-883001','2026-03','2026-02-21','2026-03-20','2026-04-10',  5000,  15200, 15000,   0.00,   0.00,   5200.00,  260.00,  152);

-- 8. Grant read-only privileges
...
-- =============================================================================
-- 9. Enforce read-only at session level
-- =============================================================================
ALTER USER rag_readonly SET default_transaction_read_only = on;

-- 10. Helpful indexes for NL2SQL / summarizer queries
...
CREATE INDEX IF NOT EXISTS idx_card_transactions_card_date
    ON card_transactions(card_id, txn_date);

CREATE INDEX IF NOT EXISTS idx_card_transactions_card_type_date
    ON card_transactions(card_id, txn_type, txn_date);

CREATE INDEX IF NOT EXISTS idx_card_transactions_category
    ON card_transactions(card_id, category_name);

CREATE INDEX IF NOT EXISTS idx_reward_transactions_card_date
    ON reward_transactions(card_id, txn_date);

CREATE INDEX IF NOT EXISTS idx_billing_statements_card_month
    ON billing_statements(card_id, billing_month);
